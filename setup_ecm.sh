...
if [ $UPGRADE_FLAG = upgrade ]; then
    ...
    apt update
    apt dist-upgrade
    apt install -y python-pip build-essential python-dev python-smbus libbluetooth-dev dialog git minicom tmux whois ntpdate pigpio python-pigpio python3-pigpio
