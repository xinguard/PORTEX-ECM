...
if [ $UPGRADE_FLAG = upgrade ]; then
    ...
    apt update
    apt -y dist-upgrade
    apt install -y python-pip python-pip3 build-essential python-dev python-smbus libbluetooth-dev dialog git minicom tmux whois ntpdate pigpio python-pigpio python3-pigpio
    apt autoremove
    apt autoclean
